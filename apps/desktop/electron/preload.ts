...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	host: {
		...
		// Main emits HOST_INSTALL_EVENT when a hosted page (Hitechhub) invokes the
	...
	hitechhub: {
		...
		// Hitechhub web surface pulls it via the hitechhub.consume-pending-share op.
		stageShare: (draft: ShareDraft): Promise<boolean> =>
			ipcRenderer.invoke("hitechhub:stage-share", draft) as Promise<boolean>,
	},
	workspace: {
		getClientConfig: () =>
			ipcRenderer.invoke(
