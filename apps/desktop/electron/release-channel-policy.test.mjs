...
test("desktop updater uses electron-updater and exposes install-now state", async () => {
  ...
  assert.match(source, /const GITHUB_RELEASES_REPO = "hitechcloudOS-releases";/);
...
test("runtime staging accepts explicit runtime sources and falls back to a locally prepared runtime bundle", async () => {
  ...
  assert.match(source, /const runtimeDir = process\.env\.HITECHCLOUD_RUNTIME_DIR\?\.trim\(\);/);
  assert.match(source, /const runtimeTarball = process\.env\.HITECHCLOUD_RUNTIME_TARBALL\?\.trim\(\);/);
  assert.match(source, /const runtimeBundleUrl = process\.env\.HITECHCLOUD_RUNTIME_BUNDLE_URL\?\.trim\(\);/);
  ...
  assert.doesNotMatch(source, /HITECHCLOUD_RUNTIME_SOURCE_REPO/);
  assert.doesNotMatch(source, /HITECHCLOUD_RUNTIME_RELEASE_TAG/);
  assert.doesNotMatch(source, /hitechcloud-vietnam\/hitechcloudOS-releases/);
...
test("manual CI workflow publishes desktop installers without standalone runtime tar assets", async () => {
  ...
  assert.match(source, /HITECHCLOUD_RELEASES_REPO: hitechcloud-vietnam\/hitechcloudOS-releases/);
  assert.match(source, /workflow_dispatch:\n\s+inputs:\n\s+ref:/);
  assert.match(source, /release_tag:\n\s+description: GitHub release tag to create in hitechcloudOS-releases/);
  assert.match(source, /release_title:\n\s+description: Optional GitHub release title \(defaults to hitechcloudOS <version>\)/);
  ...
  assert.match(source, /release_tag must match hitechcloudOS-YYYY\.MDD\.R/);
  assert.match(source, /release_version="\$\{release_tag#hitechcloudOS-\}"/);
  assert.match(source, /release_title="hitechcloudOS \$\{release_version\}"/);
  ...
  assert.match(source, /manual release publishing to \$\{RELEASE_GH_REPO\} requires HITECHCLOUD_RELEASES_REPO_TOKEN/);
  ...
  assert.match(source, /HITECHCLOUD_RUNTIME_DIR: \$\{\{ github\.workspace \}\}\/out\/runtime-macos/);
  assert.doesNotMatch(source, /HITECHCLOUD_RUNTIME_TARBALL:/);
  assert.doesNotMatch(source, /RUNTIME_ASSET_NAME: hitechcloudOS-runtime-linux\.tar\.gz/);
  assert.doesNotMatch(source, /RUNTIME_ASSET_NAME: hitechcloud-runtime-macos\.tar\.gz/);
  assert.doesNotMatch(source, /RUNTIME_ASSET_NAME: hitechcloud-runtime-windows\.tar\.gz/);
  ...
  // HITECHCLOUD_RUNTIME_TARBALL assertions above still cover the rest, and this
  ...
  assert.match(source, /packaged_app="\$\{RUNNER_TEMP\}\/hitechcloudOS\.app"/);
  ...
  assert.match(source, /macOS zip does not contain hitechcloudOS\.app as the root app bundle/);
  ...
  assert.match(source, /hitechcloudOS\.app was not extracted from the final macOS zip/);
  ...
  assert.doesNotMatch(source, /published macOS zip is missing hitechcloudOS\.app\/Contents\/Resources\/app-update\.yml/);
  ...
  assert.match(source, /name: hitechcloud-desktop-macos-\$\{\{ inputs\.release_tag \}\}/);
  assert.match(source, /Download Intel macOS desktop release artifacts/);
  assert.match(source, /name: hitechcloud-desktop-macos-intel-\$\{\{ inputs\.release_tag \}\}/);
  assert.match(source, /Download Windows desktop release artifacts/);
  assert.match(source, /name: hitechcloud-desktop-windows-\$\{\{ inputs\.release_tag \}\}/);
  ...
  assert.match(source, /RELEASE_GH_REPO: hitechcloud-vietnam\/hitechcloudOS-releases/);
  ...
  assert.match(source, /mac_dmg_asset="release-assets\/macos-desktop\/hitechcloudOS-macos-arm64\.dmg"/);
  ...
  assert.match(source, /mac_intel_dmg_asset="release-assets\/macos-intel-desktop\/hitechcloudOS-macos-x64\.dmg"/);
  ...
  assert.match(source, /windows_installer_asset="\$\{windows_release_dir\}\/hitechcloudOS-windows-x64-setup\.exe"/);
  ...
  assert.match(source, /does not reference hitechcloudOS-windows-x64-setup\.exe/);
  ...
  assert.doesNotMatch(source, /hitechcloud-runtime-windows\.tar\.gz/);
  ...
  assert.doesNotMatch(builderConfig, /HITECHCLOUD_BUNDLE_TOOLCHAIN_SEED/);
  assert.doesNotMatch(builderConfig, /HITECHCLOUD_TOOLCHAIN_TARBALL/);
  ...
  // a step should not.
  assert.match(source, /--filter=@hitechcloud\/runtime-state-store/);
  assert.match(source, /--filter=@hitechcloud\/runtime-harness-host/);
});
...
test("dmg target pins an explicit image size so dmgbuild can't undersize it", async () => {
  ...
  // avoids that; without it hitechcloudOS-2026.815.1 could not be released.
