...
test("desktop runtime uses the managed hitechcloud catalog instead of local seed catalogs or local suppression", async () => {
  ...
  assert.doesNotMatch(source, /isUnsupportedHitechcloudRuntimeModel\(/);
  assert.doesNotMatch(source, /seedLegacyHitechcloudProxyModels/);
  assert.doesNotMatch(source, /RUNTIME_HITECHCLOUD_LEGACY_PROXY_MODELS/);
});
...
test("desktop model catalog carries reasoning metadata and the composer persists thinking preferences", async () => {
  ...
  assert.match(sharedCatalogSource, /hitechcloud_model_proxy:\s*\{[\s\S]*source: "backend"/);
  assert.match(sharedCatalogSource, /hitechcloud_model_proxy:\s*\{[\s\S]*"gpt-5\.5"/);
  ...
  assert.match(mainSource, /function managedHitechcloudRuntimeModelConfig\(/);
