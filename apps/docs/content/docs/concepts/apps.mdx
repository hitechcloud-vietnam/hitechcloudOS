...
description: Why the app layer exists and how it fits into the hitechcloudOS system.
---

An app is the packaged capability unit that a workspace installs.

It is how `hitechcloudOS` gives a workspace durable, inspectable, domain-specific behavior without collapsing that behavior into the harness, the prompt, or the desktop shell.
...
`hitechcloudOS` is not only a runtime and memory system. It also needs a stable way to package capability.
...
Without the app layer, `hitechcloudOS` would have an environment model but no first-class unit for packaged business or workflow capability.
...
`Apps` and `Plugins` are separate systems in `hitechcloudOS`.
...
Once a capability needs lifecycle, state, integrations, UI, MCP exposure, and outputs to stay coordinated over time, `hitechcloudOS` needs one coherent unit to own that outer contract. That is the role of the app layer.

An app can still be thin. It may depend on one integration, expose only a small MCP tool surface, or internally rely on a CLI. `hitechcloudOS` models that unit as an app when the workspace and runtime need to manage those pieces as one installed capability over time.
...
For builders, the app layer gives `hitechcloudOS` a clean extension model:
