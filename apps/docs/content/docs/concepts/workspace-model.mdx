...
Workspaces live under the runtime OS root. In desktop mode that root comes from the local host data directory. In standalone runtime deployments it defaults to `/hitechcloud`.
...
`<workspace-id>/.hitechcloud/` now holds the workspace bundle's runtime-owned surfaces:
...
Workspace-scoped markdown memory lives inside each workspace at `<workspace-id>/.hitechcloud/memory/`, while integration memory and profile-global memory live under the shared `memory/` root.
