...
description: Implementation details of hitechcloudOS Desktop.
---
...
- `apps/desktop/shared/model-catalog.ts`: shipped fallback model metadata for direct providers and hitechcloudOS Proxy model mapping, including reasoning support, allowed thinking values, and input modalities.
...
1. Resolve the staged runtime bundle under `apps/desktop/out/runtime-<platform>` or an override such as `HITECHCLOUD_RUNTIME_ROOT`.
...
3. Spawn `bin/sandbox-runtime` with desktop-owned env such as `HB_SANDBOX_ROOT`, `SANDBOX_AGENT_BIND_HOST=127.0.0.1`, `SANDBOX_AGENT_BIND_PORT=<resolved runtime port>`, `SANDBOX_AGENT_HARNESS`, `HITECHCLOUD_HOST_STATE_DB_PATH`, `HITECHCLOUD_CONTROL_PLANE_DB_PATH`, the legacy `HITECHCLOUD_RUNTIME_DB_PATH` alias, and the bridge settings.
...
The embedded runtime port is not fixed anymore. `apps/desktop/electron/main.ts` resolves it from `HITECHCLOUD_RUNTIME_API_PORT` or, when unset, from the resolved Electron `userData` path. Port `5060` is explicitly rejected, which is why older fixed-port assumptions are no longer reliable for diagnostics.
...
`HITECHCLOUD_RUNTIME_ROOT` can override the staged bundle location when you need the desktop to boot a different packaged runtime tree.
...
`createWorkspace()` in `apps/desktop/electron/main.ts` now also emits structured stage logs with the `[hitechcloud.createWorkspace]` prefix for debugging create failures across materialization, runtime record creation, template apply, and workspace activation.
...
- `apps/desktop/electron/main.ts` refreshes a managed runtime model catalog from the control plane when a signed-in hitechcloudOS session is active, caches that catalog, and merges it with the locally persisted `runtime-config.json`.
...
If a hitechcloudOS-managed proxy model should inherit local fallback metadata, also update the proxy mapping rules in `mappedHitechcloudProxyProviderModel(...)`:
...
- Use `bash apps/desktop/scripts/check-runtime-status.sh` when the embedded runtime fails to start, a workspace looks corrupted, or app lifecycle behavior diverges from the desktop UI. The helper now follows the same `HITECHCLOUD_RUNTIME_API_PORT` and `userData`-path-derived port rules as the desktop itself.
