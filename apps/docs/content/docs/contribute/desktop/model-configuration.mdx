...
description: How hitechcloudOS Desktop configures model providers and routing.
---

![Model Settings](/images/model-settings.png)

hitechcloudOS ships with a default model setup. In most cases, you do not need to edit `runtime-config.json` by hand, because the desktop app already exposes the main configuration surfaces for provider connection, managed model catalogs, background tasks, recall embeddings, image generation, and web search.
...
| --- | --- | --- |
| `hitechcloud_proxy` | hitechcloudOS-managed setups | Uses the hitechcloudOS runtime binding and managed model catalog instead of a local seed list. |
...
Desktop currently exposes only hitechcloudOS Proxy in `Settings -> Model Providers`. The broader provider kinds below still describe lower-level runtime capabilities and legacy config shapes, but they are no longer first-class desktop provider choices.

## In-app setup

hitechcloudOS already provides model configuration in the desktop app:

- open `Settings -> Model Providers`
- connect hitechcloudOS Proxy
...
- use the `Web search` panel to choose Hitechcloud Search or Exa and provide a search API key when needed
- saved changes update `runtime-config.json`, and the chat model picker uses the configured provider models

hitechcloudOS-managed sessions can inject managed background, embedding, and image defaults through the runtime binding. The desktop UI now assumes hitechcloudOS Proxy is the only provider surface and derives its model suggestions from the managed catalog plus the shipped Hitechcloud fallback metadata.

## Catalog-driven chat models

The current desktop no longer treats chat models as only freeform strings.

- The shipped fallback catalog in `apps/desktop/shared/model-catalog.ts` now describes hitechcloudOS Proxy models only.
- Signed-in hitechcloudOS-managed sessions can also fetch a managed runtime model catalog from the control plane.
...
| --- | --- |
| `hitechcloud_model_proxy/gpt-5.4` | `none`, `low`, `medium`, `high`, `xhigh` |
| `hitechcloud_model_proxy/gpt-5.5` | `none`, `low`, `medium`, `high`, `xhigh` |
| `hitechcloud_model_proxy/gpt-5.3-codex` | `low`, `medium`, `high`, `xhigh` |

The managed hitechcloudOS catalog can provide explicit reasoning metadata. If it does not, the desktop falls back to the shipped Hitechcloud proxy metadata for known GPT-5 family models.
...
- `hitechcloud_model_proxy`: `text-embedding-3-small`
...
- `hitechcloud_model_proxy`: managed catalog / runtime-binding defaults when available
...
- `hitechcloud_search`
  ...
  - production fallback endpoint: `https://api.hitechcloud.vn/api/v1/search/web`
  - API key is optional when the runtime has a signed-in Hitechcloud binding
- `exa`
  ...
  "web_search": {
    ...
    "providers": {
      "hitechcloud_search": {
        ...
        "base_url": "https://api.hitechcloud.vn/api/v1/search/web"
      },
      "exa": {
        "kind": "exa_hosted_mcp",
...
```bash
export HITECHCLOUD_MODEL_PROXY_BASE_URL="https://your-proxy.example/api/v1/model-proxy"
export HITECHCLOUD_SANDBOX_AUTH_TOKEN="your-proxy-token"
export HITECHCLOUD_DEFAULT_MODEL="anthropic/claude-sonnet-4-6"
```
