...
- The embedded desktop runtime binds to `127.0.0.1:<derived-port>`. `apps/desktop/electron/main.ts` resolves that port from `HITECHCLOUD_RUNTIME_API_PORT` or, when unset, from a hash of the Electron `userData` path. Port `5060` is rejected, and `5160` is only the bootstrap fallback before per-profile derivation runs.
...
curl -X POST http://127.0.0.1:${RUNTIME_PORT}/api/v1/capabilities/runtime-tools/reports \
  -H 'x-hitechcloud-workspace-id: workspace-1' \
  -H 'x-hitechcloud-session-id: session-main' \
  -H 'x-hitechcloud-input-id: input-1' \
  -H 'x-hitechcloud-selected-model: openai_direct/gpt-5.4' \
...
- `GET /api/v1/apps/:appId/setup-log?workspace_id=...` returns the latest setup log tail plus recent structured events from `<workspace>/apps/<appId>/.hitechcloud/logs/`.
