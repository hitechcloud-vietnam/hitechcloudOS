...
`@hitechcloud/runtime-state-store` owns the runtime's durable SQLite surfaces.
...
- `HITECHCLOUD_HOST_STATE_DB_PATH` if set
- else `HITECHCLOUD_RUNTIME_DB_PATH` as a legacy alias
- else `<sandboxRoot>/state/host-state.db`

`controlPlaneDbPath()` defaults to:

- `HITECHCLOUD_CONTROL_PLANE_DB_PATH` if set
- else `<sandboxRoot>/state/control-plane.db`

Workspace execution state lives at:

- `<workspaceRoot>/<workspace-id>/.hitechcloud/state/runtime.db`

The store also resolves workspace paths from:

- the workspace registry row
- the workspace identity file under `.hitechcloud/state/workspace_id`
...
- workspace-scoped interaction memory lives on disk under each workspace's `.hitechcloud/memory/interaction/`
- workspace-scoped semantic interaction trees live under `.hitechcloud/memory/semantic/interaction/trees/`
- workspace runtime continuity memory lives separately under `.hitechcloud/memory/runtime/`
...
- `<workspace-id>/.hitechcloud/state/runtime.db` for workspace execution truth and workspace-scoped runtime metadata
- workspace files for authored config and app/template state
- `.hitechcloud/memory/interaction/` plus `.hitechcloud/memory/semantic/interaction/trees/` under each workspace for durable interaction memory and its derived semantic trees
- `.hitechcloud/memory/runtime/` under each workspace for runtime continuity memory that helps runs resume coherently
...
- `.hitechcloud/state/` under each workspace for workspace-local runtime state such as the identity marker and harness session mapping
...
The package exposes a CLI entrypoint, `hitechcloud-state-store`, backed by `runtime/state-store/src/cli.ts`.
...
hitechcloud-state-store claim-inputs --request-base64 "$(printf '%s' "$REQ" | base64 | tr -d '\n')"
```
