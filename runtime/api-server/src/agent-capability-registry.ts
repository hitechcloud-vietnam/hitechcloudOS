...
export function renderCapabilityToolRoutingPromptSection(
  ...
  if (manifest.runtime_tools.some((capability) => capability.id === "hitechcloud_workspace_integrations_propose_connect")) {
    ensureHeading();
    lines.push("Integration routing: when the user references their OWN account on a known third-party service (their Twitter posts, their Gmail inbox, their GitHub repos, their Notion pages, their Slack channels, etc.) and the provider's `<toolkit>_<verb>` tools are NOT in your current tool list, call `hitechcloud_workspace_integrations_propose_connect` for that toolkit FIRST. The chat UI renders a one-click Connect card from the result.");
