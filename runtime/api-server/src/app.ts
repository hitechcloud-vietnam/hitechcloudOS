...
export function buildRuntimeApiServer(options: BuildRuntimeApiServerOptions = {}): FastifyInstance {
  ...
  app.post(
    "/api/v1/capabilities/runtime-tools/hitechhub-upload-image",
    async (request, reply) => {
      if (!isRecord(request.body)) {
        return sendError(reply, 400, "request body must be an object");
      }
      try {
        ...
        return await runtimeAgentToolsService.hitechhubUploadImage({
          workspaceId,
          path: requiredString(request.body.path, "path"),
        });
      } catch (error) {
        ...
        return sendError(
          reply,
          400,
          error instanceof Error ? error.message : "hitechhub_upload_image failed",
        );
      }
    },
  );
