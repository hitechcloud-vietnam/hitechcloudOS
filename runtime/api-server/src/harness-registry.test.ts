...
const ORIGINAL_ENV = {
  ...
  HITECHCLOUD_SUBAGENT_HARNESS_RUN_TIMEOUT_S: process.env.HITECHCLOUD_SUBAGENT_HARNESS_RUN_TIMEOUT_S,
  HITECHCLOUD_TASK_PROPOSAL_HARNESS_RUN_TIMEOUT_S: process.env.HITECHCLOUD_TASK_PROPOSAL_HARNESS_RUN_TIMEOUT_S
};
...
afterEach(() => {
  if (ORIGINAL_ENV.HITECHCLOUD_HARNESS_RUN_TIMEOUT_S === undefined) {
    ...
    process.env.HITECHCLOUD_HARNESS_RUN_TIMEOUT_S = ORIGINAL_ENV.HITECHCLOUD_HARNESS_RUN_TIMEOUT_S;
  }
  if (ORIGINAL_ENV.HITECHCLOUD_SUBAGENT_HARNESS_RUN_TIMEOUT_S === undefined) {
    delete process.env.HITECHCLOUD_SUBAGENT_HARNESS_RUN_TIMEOUT_S;
  } else {
    process.env.HITECHCLOUD_SUBAGENT_HARNESS_RUN_TIMEOUT_S =
      ORIGINAL_ENV.HITECHCLOUD_SUBAGENT_HARNESS_RUN_TIMEOUT_S;
  }
  if (ORIGINAL_ENV.HITECHCLOUD_TASK_PROPOSAL_HARNESS_RUN_TIMEOUT_S === undefined) {
    delete process.env.HITECHCLOUD_TASK_PROPOSAL_HARNESS_RUN_TIMEOUT_S;
  } else {
    process.env.HITECHCLOUD_TASK_PROPOSAL_HARNESS_RUN_TIMEOUT_S = ORIGINAL_ENV.HITECHCLOUD_TASK_PROPOSAL_HARNESS_RUN_TIMEOUT_S;
  }
});
...
test("requireRuntimeHarnessPlugin uses extended timeouts for subagent runs", () => {
  ...
  process.env.HITECHCLOUD_SUBAGENT_HARNESS_RUN_TIMEOUT_S = "900";
