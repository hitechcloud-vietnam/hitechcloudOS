...
test("workspaceDataDbPath places data.db under the workspace `.hitechcloud/state` folder", () => {
  const workspaceDir = path.join(os.tmpdir(), "hb-ts-runner-data-db-test");
  const dbPath = workspaceDataDbPath(workspaceDir);
  assert.equal(dbPath, path.join(path.resolve(workspaceDir), ".hitechcloud", "state", "data.db"));
});
